...
import { generateProjectPlan, generateImage, generateSpeech, continueProjectPlan, generateConclusion, generatePromptFromImage, AVAILABLE_VOICES } from '../services/geminiService';
...
const WorkflowManager: React.FC = () => {
  ...
  const [isZipping, setIsZipping] = useState<'images' | 'audio' | 'prompts' | 'all' | null>(null);
  ...
  const handleGenerateVideoPrompt = async (asset: GeneratedAsset) => {
    if (!asset.imageUrl) return;
    updateAsset(asset.scene, { videoPromptStatus: 'generating' });
    try {
      const prompt = await generatePromptFromImage(asset.imageUrl);
      updateAsset(asset.scene, { videoPrompt: prompt, videoPromptStatus: 'done' });
    } catch (err) {
      console.error("Failed to generate video prompt:", err);
      updateAsset(asset.scene, { videoPromptStatus: 'error' });
    }
  };
  ...
  const generateAssetsForScenes = async (scenes: Scene[]): Promise<GeneratedAsset[]> => {
      ...
      const assetPromises = scenes.map(async (scene): Promise<GeneratedAsset> => {
          ...
          return { scene: scene.scene, narration: scene.narration, imagePrompt: scene.imagePrompt, imageUrl, audioUrl, videoPromptStatus: 'idle' };
      });
  ...
  const handleDownloadAll = async (type: 'images' | 'audio' | 'prompts') => {
    setIsZipping(type);
    try {
        const zip = new JSZip();
        ...
        const promises = generatedAssets.map(async (asset) => {
            ...
            } else if (type === 'prompts' && asset.videoPrompt) {
                zip.file(`scene_${asset.scene}_prompt.txt`, asset.videoPrompt);
            }
        });
 ...
 const handleDownloadAllAssets = async () => {
    ...
    try {
        ...
        const promises = generatedAssets.map(async (asset) => {
            ...
            // Add video prompt text
            if (asset.videoPrompt) {
                sceneFolder.file('video_prompt.txt', asset.videoPrompt);
            }
                 ...
                 <div className="max-h-[480px] overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-2">
                    {generatedAssets.sort((a,b) => a.scene - b.scene).map(asset => 
                      <SceneCard 
                        ...
                        onGenerateVideoPrompt={() => handleGenerateVideoPrompt(asset)} 
                 ...
                 <div className="mt-6 border-t border-gray-700 pt-6">
                    ...
                    <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-700">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <button onClick={() => handleDownloadAll('prompts')} disabled={!!isZipping} className="flex items-center justify-center gap-2 bg-purple-600 text-white text-sm font-bold py-2 px-3 rounded-lg hover:bg-purple-700 disabled:bg-purple-900/50 disabled:cursor-not-allowed">
                          {isZipping === 'prompts' ? <Spinner className="w-4 h-4" /> : <DownloadIcon className="w-4 h-4" />} All Prompts (.zip)
                        </button>
                      </div>
                      <p className="text-xs text-gray-500 text-center mt-3">
                        To create a video, download the assets and combine them in your favorite video editing software.
                      </p>
                    </div>
                 </div>
            </div>
