...
export const generateVideoFromImage = async (prompt: string, image: Part): Promise<string> => {
    if (!image.inlineData?.data || !image.inlineData.mimeType) {
        throw new Error("Invalid image part provided for video generation. It must have inlineData with data and mimeType.");
    }
    ...
    if (!downloadLink) {
        ...
        throw new Error("Video generation completed but no download URI was provided.");
    }
    ...
    if (!videoResponse.ok) {
        ...
        console.error("Failed to download video. Status:", videoResponse.status, "Response:", errorText);
